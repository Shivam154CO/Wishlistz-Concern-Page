...
const UserConcerns: React.FC<UserConcernsProps> = ({ concerns, userEmail, onBack }) => {
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm">
              ...
              <span className="ml-2 font-medium text-gray-900">{userEmail}</span>
            </div>
          </div>
        </div>
        ...
        {concerns.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-xl p-12 text-center">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              You haven't submitted any support tickets yet. Click the button below to create your first ticket.
            </p>
            <button
              onClick={onBack}
          ...
          <div className="space-y-6">
            {concerns.map((concern) => (
              <div key={concern.id} className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
                {/* Header */}
                <div className="p-6 border-b border-gray-100">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="text-lg font-bold text-gray-900">{concern.ticketId}</h3>
                          {getStatusBadge(concern.status)}
                        </div>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-600">
                          <span className="font-medium">{concern.category}</span>
                    ...
                    <div className="lg:text-right">
                      ...
                      <div className="font-mono font-bold text-gray-900 text-lg">{concern.ticketId}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                      <div className="mb-6">
                        <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Issue Description</h4>
                      ...
                      {concern.adminNotes && (
                        <div>
                          <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Admin Response</h4>
                          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                            <p className="text-gray-700 whitespace-pre-line">{concern.adminNotes}</p>
                    ...
                    <div>
                      {/* Status Timeline */}
                      <div className="mb-6">
                        <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Ticket Status</h4>
                        <div className="space-y-4">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">{formatDate(concern.createdAt)}</p>
                            </div>
                          </div>
                          
                          <div className="flex items-start gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                              concern.status === 'New' ? 'bg-gray-100' : 
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                {concern.status === 'New' ? 'Pending' : 'Being reviewed by support'}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                              concern.status === 'Resolved' ? 'bg-green-100' : 
                              concern.status === 'In-progress' ? 'bg-yellow-100' : 'bg-gray-100'
                            }`}>
                              <span className="text-sm">
                                {concern.status === 'Resolved' ? '✅' : 
                                 concern.status === 'In-progress' ? '⚙️' : '⏳'}
                              </span>
                            </div>
                            <div>
                              <p className="font-medium text-gray-900">
                                {concern.status === 'Resolved' ? 'Resolved' : 
                                 concern.status === 'In-progress' ? 'In Progress' : 'Pending Resolution'}
                              </p>
                              <p className="text-sm text-gray-500">
                                {concern.status === 'Resolved' ? 'Issue has been resolved' : 
                                 concern.status === 'In-progress' ? 'Working on solution' : 'Awaiting action'}
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* Attachments */}
                      <div>
                        <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Attachments</h4>
                        <div className="space-y-2">
                          {concern.screenshots.map((_, index) => (
                            <div key={index} className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm">
                      ...
                      <span className="font-medium text-gray-900">{formatDate(concern.createdAt)}</span>
                    </div>
                  </div>
                </div>
              </div>
